...
import { WhistleIcon } from "@/components/assistant-ui/whistle-icon";
...
function ToolGroupTrigger({
  ...
  return (
    <CollapsibleTrigger
      ...
      className={cn(
        "aui-tool-group-trigger group/trigger text-muted-foreground hover:text-foreground flex w-fit items-center gap-1.5 text-sm transition-colors",
        className,
      )}
      {...props}
    >
      {active ? (
        <SoccerBallSpinner
          ...
          className="aui-tool-group-trigger-loader size-9 shrink-0"
        />
      ) : (
        <WhistleIcon className="size-9 shrink-0" />
      )}
      <span
...
function ToolGroupContent({
  ...
  return (
    <CollapsibleContent
      ...
      {...props}
    >
      <div className="border-border/60 mt-2 ml-1.5 flex flex-col gap-3 border-l pl-4">
        {children}
      </div>
    </CollapsibleContent>
  );
